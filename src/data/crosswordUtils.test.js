...
// ===========================================
// Crossword - parseWordlistData Tests
// ===========================================
describe('Crossword - parseWordlistData', () => {
  it('should parse wordlist with one word per line', () => {
    const rawData = 'apple\nbanana\norange';
    const entries = parseWordlistData(rawData);

    expect(entries).toHaveLength(3);
    expect(entries[0].answer).toBe('APPLE');
    expect(entries[1].answer).toBe('BANANA');
    expect(entries[2].answer).toBe('ORANGE');
  });
  ...
  it('should convert answers to uppercase', () => {
    ...
    expect(entries[0].answer).toBe('LOWER');
  });

  it('should remove underscores from multi-word entries', () => {
    const rawData = 'ice_cream';
    const entries = parseWordlistData(rawData);

    expect(entries[0].answer).toBe('ICECREAM');
  });
  ...
  it('should filter answers to 3-12 characters', () => {
    ...
    expect(entries[0].answer).toBe('CAT');
  });
  ...
  it('should preserve original word in entry', () => {
    const rawData = 'ice_cream';
    const entries = parseWordlistData(rawData);

    expect(entries[0].originalWord).toBe('ice_cream');
  });
});

// ===========================================
// Crossword - parseCluesData Tests (wordlist-based)
// ===========================================
describe('Crossword - parseCluesData', () => {
  it('should generate clues from wordlist', () => {
    const rawData = 'apple\nbanana';
    const clues = parseCluesData(rawData);

    expect(clues).toHaveLength(2);
    expect(clues[0]).toHaveProperty('clue');
    expect(clues[0]).toHaveProperty('answer');
    expect(clues[0]).toHaveProperty('direction');
  });

  it('should generate clues that mention word length', () => {
    const rawData = 'apple';
    const clues = parseCluesData(rawData);

    // Clue should reference word length
    expect(clues[0].clue).toMatch(/5/);
  });

  it('should assign alternating directions', () => {
    const rawData = 'one\ntwo\nthree';
    const clues = parseCluesData(rawData);

    expect(clues.map(c => c.direction)).toEqual(['across', 'down', 'across']);
  });
});
