...
export function buildLookup(regionConfig) {
  ...
  regionConfig.regions.forEach(region => {
    // Primary name
    lookup[region.name.toLowerCase()] = region.code;
    // Code itself
    lookup[region.code.toLowerCase()] = region.code;
    // Aliases
    if (region.aliases) {
      region.aliases.forEach(alias => {
        lookup[alias.toLowerCase()] = region.code;
      });
    }
  });
  return lookup;
...
export function getRegionCode(feature, regionConfig) {
