...
// ===========================================
// Anagrams - Letter Usage Helpers
// ===========================================
describe('Anagrams - letter usage helpers', () => {
  const letters = ['A', 'N', 'A', 'G', 'R', 'A', 'M'];

  it('should allow adding letters until copies are exhausted', () => {
    let word = '';
    word = appendLetterIfAvailable(word, 'A', letters);
    expect(word).toBe('A');
    ...
    word = appendLetterIfAvailable(word, 'A', letters);
    expect(word).toBe('AAA');

    // No more A tiles available
    word = appendLetterIfAvailable(word, 'A', letters);
    expect(word).toBe('AAA');
  });

  it('should report when a letter is no longer available', () => {
    expect(canUseLetter('AAA', 'A', letters)).toBe(false);
    expect(canUseLetter('ANAGRA', 'M', letters)).toBe(true);
  });

  it('should ignore invalid or unsupported letters', () => {
    expect(canUseLetter('AAA', '1', letters)).toBe(false);
    expect(appendLetterIfAvailable('AAA', '1', letters)).toBe('AAA');
  });
});
