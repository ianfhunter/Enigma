...
export default function CapitalGuesser() {
  ...
  const setupRound = useCallback(() => {
    let country;
    if (mode === 'daily') {
      country = getDailyCountry(getTodayString() + '-capital');
    } else {
      country = getRandomCountry();
    }

    const capital = getCapital(country.code);
    setCurrentCountry({ ...country, capital });
    setOptions(getRandomCapitalOptions(capital, 3));
