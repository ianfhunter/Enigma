...
const getConstellationOptions = (correct, count = 3) => {
  const allConstellations = constellationData.constellations.map(c => c.names[0].english);
  const wrongOptions = allConstellations.filter(c => c !== correct);
...
const QUESTION_TYPES = [
  {
    ...
    getOptions: (c) => getConstellationOptions(c.names[0].english),
  },
  {
    id: 'abbreviation',
    ...
    getAnswer: (c) => c.IAU,
    getOptions: (c) => {
      const allAbbrevs = constellationData.constellations.map(con => con.IAU);
      const wrong = shuffle(allAbbrevs.filter(a => a !== c.IAU)).slice(0, 3);
      return shuffle([c.IAU, ...wrong]);
    },
  },
  {
    id: 'semantic',
    ...
    getOptions: (c) => {
      const allSemantics = [...new Set(constellationData.constellations.flatMap(con => con.semantics || []))];
      const answer = c.semantics?.[0] || 'unknown';
      const wrong = shuffle(allSemantics.filter(s => s !== answer)).slice(0, 3);
...
export default function Constellations() {
  ...
  const constellations = useMemo(() => constellationData.constellations, []);
  ...
  const setupRound = useCallback(() => {
    ...
    setOptions(qType.getOptions(constellation));
  ...
  useEffect(() => {
    if (mode && !gameOver && !currentConstellation) {
      setupRound();
    }
  }, [mode, gameOver, currentConstellation, setupRound]);
      ...
      <div className={styles.container}>
        ...
        <div className={styles.menuArea}>
          ...
          <div className={styles.dataInfo}>
            <span>⭐ {constellations.length} official IAU constellations</span>
          </div>
        </div>
      </div>
    );
