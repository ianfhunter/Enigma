...
export function recycleWasteToStock(stock, waste, hasRecycledStock) {
  if (stock.length > 0 || waste.length === 0 || hasRecycledStock) {
    return { stock, waste, recycled: hasRecycledStock, changed: false };
  }
...
export default function CrazyQuilt() {
  ...
  const [hasRecycledStock, setHasRecycledStock] = useState(false);
  ...
  const drawFromStock = () => {
    ...
    if (stock.length === 0) {
      const recycled = recycleWasteToStock(stock, waste, hasRecycledStock);
      if (!recycled.changed) return;
      setStock(recycled.stock);
      setWaste(recycled.waste);
      setHasRecycledStock(recycled.recycled);
      setSelected(null);
      return;
    }
      ...
      <section className={styles.tableauArea}>
        <div className={styles.sidePiles}>
          <button
            ...
            disabled={stock.length === 0 && (waste.length === 0 || hasRecycledStock)}
          >
            <span className={styles.pileLabel}>{t('crazyQuilt.stock')}</span>
            <span>{stock.length || '—'}</span>
            {stock.length === 0 && waste.length > 0 && hasRecycledStock && (
              <small className={styles.recycleUsed}>{t('crazyQuilt.recycleUsed')}</small>
            )}
          </button>
