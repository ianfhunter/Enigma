...
describe('CrazyQuilt foundation display safety', () => {
  it('returns fallback A/K display cards when piles are temporarily empty', () => {
    const foundations = {
      spades: { up: [], down: [] },
    };

    const up = getFoundationDisplayCard(foundations, 'spades', 'up');
    const down = getFoundationDisplayCard(foundations, 'spades', 'down');

    expect(up.rank).toBe('A');
    expect(up.suitSymbol).toBe('♠');
    expect(down.rank).toBe('K');
    expect(down.suitSymbol).toBe('♠');
  });

  it('returns the actual top card when pile has cards', () => {
    const foundations = {
      hearts: {
        up: [makeCard('hearts', 'A', 0), makeCard('hearts', '2', 1)],
        down: [makeCard('hearts', 'K', 12), makeCard('hearts', 'Q', 11)],
      },
    };

    expect(getFoundationDisplayCard(foundations, 'hearts', 'up').rank).toBe('2');
    expect(getFoundationDisplayCard(foundations, 'hearts', 'down').rank).toBe('Q');
  });
});


describe('CrazyQuilt stock recycle rule', () => {
  it('recycles waste to stock once, preserving order and then blocks further recycle', () => {
    const c1 = makeCard('spades', '2', 1);
    const c2 = makeCard('hearts', '3', 2);
    const c3 = makeCard('clubs', '4', 3);

    const first = recycleWasteToStock([], [c1, c2, c3], false);
    expect(first.changed).toBe(true);
    expect(first.stock).toEqual([c3, c2, c1]);
    expect(first.waste).toEqual([]);
    expect(first.recycled).toBe(true);

    const second = recycleWasteToStock([], [c1], true);
    expect(second.changed).toBe(false);
    expect(second.stock).toEqual([]);
    expect(second.waste).toEqual([c1]);
    expect(second.recycled).toBe(true);
  });

  it('does not recycle when stock still has cards or waste is empty', () => {
    const c1 = makeCard('spades', '2', 1);

    const withStock = recycleWasteToStock([c1], [c1], false);
    expect(withStock.changed).toBe(false);

    const noWaste = recycleWasteToStock([], [], false);
    expect(noWaste.changed).toBe(false);
  });
});
