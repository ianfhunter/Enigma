...
const getRandomOptions = (correct, field, count = 3) => {
  const allValues = [...new Set(paintingsData.map(p => p[field]).filter(Boolean))];
...
const getRandomPainting = (exclude = []) => {
  const available = paintingsData.filter(p => !exclude.includes(p.title));
...
const QUESTION_TYPES = [
  {
    ...
    answer: (p) => p.artist,
    options: (p) => getRandomOptions(p.artist, 'artist'),
    showImage: true
  },
  {
    id: 'title',
    ...
    answer: (p) => p.title,
    options: (p) => getRandomOptions(p.title, 'title'),
    showImage: true
  },
  {
    id: 'year',
    ...
    answer: (p) => p.year,
    options: (p) => {
      const years = paintingsData.map(p => p.year).filter(Boolean);
  ...
  },
  {
    ...
    answer: (p) => p.style,
    options: (p) => getRandomOptions(p.style, 'style'),
    showImage: true
  },
  {
    id: 'location',
    ...
    answer: (p) => p.location,
    options: (p) => getRandomOptions(p.location, 'location'),
    showImage: false
  }
];
...
export default function FamousPaintings() {
  ...
  useEffect(() => {
    if (mode && !gameOver && !currentPainting) {
      setupRound();
    }
  }, [mode, gameOver, currentPainting, setupRound]);
      ...
      <div className={styles.container}>
        ...
        <div className={styles.menuArea}>
          ...
          <div className={styles.dataInfo}>
            <span>🖼️ {paintingsData.length} masterpieces from world-famous artists</span>
          </div>
        </div>
      </div>
    );
