...
export default function Fifteen() {
  ...
  useEffect(() => {
    const onKeyDown = (e) => {
      ...
      const { r, c } = idxToRC(blankIdx, size);
      let tileIdx = null;
      // Arrow keys: move a tile in that direction into the blank (blank moves opposite)
      if (e.key === 'ArrowLeft' && c < size - 1) tileIdx = rcToIdx(r, c + 1, size);
      if (e.key === 'ArrowRight' && c > 0) tileIdx = rcToIdx(r, c - 1, size);
      if (e.key === 'ArrowUp' && r < size - 1) tileIdx = rcToIdx(r + 1, c, size);
      if (e.key === 'ArrowDown' && r > 0) tileIdx = rcToIdx(r - 1, c, size);
      if (tileIdx != null) {
        e.preventDefault();
  ...
  }, [board, pushBoard, size, history.length]);
