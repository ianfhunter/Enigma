...
export default function GodsQuiz() {
  ...
  const gods = useMemo(() => {
    if (!data?.gods) return [];
    if (mythology === 'all') return data.gods;
    return data.gods.filter(g => g.mythology === mythology);
  }, [data, mythology]);
  ...
  const submit = () => {
    ...
    const guessed = Array.from(guessDomains);
    
    const isCorrect = 
      guessed.length === correctDomains.length &&
      guessed.every(d => correctDomains.includes(d));
