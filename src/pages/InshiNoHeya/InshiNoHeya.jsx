...
export default function InshiNoHeya() {
  ...
  useEffect(() => {
    ...
    if (showErrors) {
      setErrors(findErrors(grid, puzzleData.roomGrid, puzzleData.rooms, size));
    } else {
      setErrors(new Set());
    }
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <p className={styles.instructions}>
          Fill in numbers 1-{size} so each row and column has each number once. 
      ...
      <div className={styles.gameArea}>
        ...
        >
          {grid.map((row, r) =>
            row.map((num, c) => {
              const borders = getRoomBorders(r, c, puzzleData.roomGrid, size);
        ...
        <div className={styles.hint}>
          <p>Use keyboard: Arrow keys to move, 1-{size} to enter, Backspace to clear</p>
        </div>
      </div>
    </div>
  );
