...
export function generateRegionMap(random = Math.random) {
  ...
  let regionMap = transformMap(BASE_REGION_MAP, transforms[Math.floor(random() * transforms.length)]);

  const labels = shuffle([0, 1, 2, 3, 4, 5, 6, 7, 8], random);
  regionMap = regionMap.map(row => row.map(cell => labels[cell]));
...
export function generatePuzzle(difficulty = 'medium', seed = Date.now()) {
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    const solution = solveGrid(emptyGrid, regionMap, random);
  ...
  return (
    <div className={styles.container}>
      <GameHeader
        ...
        instructions={t('jigsawSudoku.instructions', { defaultValue: 'Classic sudoku rules, but with irregular 9-cell regions.' })}
      />
      ...
      <div className={styles.topBar}>
        <Timer isRunning={gameState === 'playing'} isPaused={gameState !== 'playing'} resetTrigger={seed} />
        ...
        {grid.map((row, r) => row.map((value, c) => {
          ...
          return (
            <button
              key={`${r}-${c}`}
              type="button"
              className={`${styles.cell} ${selectedCell ? styles.selected : ''} ${fixed ? styles.fixed : ''}`}
              data-region={region}
