        ...
        {grid.map((row, r) => row.map((value, c) => {
          ...
          return (
            <button
              key={`${r}-${c}`}
              type="button"
              className={`${styles.cell} ${styles[`region${region}`]} ${r % 3 === 0 ? styles.boxTop : ''} ${c % 3 === 0 ? styles.boxLeft : ''} ${r === SIZE - 1 ? styles.boxBottom : ''} ${c === SIZE - 1 ? styles.boxRight : ''} ${selectedCell ? styles.selected : ''} ${fixed ? styles.fixed : ''}`}
              data-region={region}
