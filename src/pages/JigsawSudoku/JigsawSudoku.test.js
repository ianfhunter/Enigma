...
describe('JigsawSudoku keyboard helpers', () => {
  it('converts keyboard digits safely', () => {
    expect(keyToDigit('1')).toBe(1);
    expect(keyToDigit('9')).toBe(9);
    expect(keyToDigit('0')).toBeNull();
    expect(keyToDigit('a')).toBeNull();
  });

  it('finds first editable cell and skips fixed cells when navigating', () => {
    const fixedCells = new Set(['0-0', '0-1', '0-2']);
    expect(findFirstEditableCell(fixedCells)).toEqual({ row: 0, col: 3 });

    const moved = moveSelection({ row: 0, col: 0 }, 'ArrowRight', fixedCells);
    expect(moved).toEqual({ row: 0, col: 3 });

    const blocked = moveSelection({ row: 0, col: 8 }, 'ArrowRight', fixedCells);
    expect(blocked).toEqual({ row: 0, col: 8 });
  });
});
