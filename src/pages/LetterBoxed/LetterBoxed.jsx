...
import { isValidWord, createSeededRandom, getTodayDateString, stringToSeed, getAllWords } from '../../data/wordUtils';
import styles from './LetterBoxed.module.css';
...
function isValidLetterBoxedWord(word, sides, lastLetter = null) {
...
export default function LetterBoxed() {
  ...
  const [stats, setStats] = useState(() => {
    const saved = localStorage.getItem('letterboxed-stats');
  ...
  const initGame = useCallback((isNewPuzzle = false) => {
    ...
    const seed = stringToSeed(`letterboxed-${today}-${puzzleOffset}`);
  ...
  useEffect(() => {
    localStorage.setItem('letterboxed-stats', JSON.stringify(stats));
  }, [stats]);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <h1 className={styles.title}>Letter Boxed</h1>
      ...
      <div className={styles.gameArea}>
        ...
        <div className={styles.inputArea}>
          <div className={styles.wordDisplay}>
            {currentWord || (lastLetter ? `Start with ${lastLetter}...` : 'Click letters...')}
          </div>
