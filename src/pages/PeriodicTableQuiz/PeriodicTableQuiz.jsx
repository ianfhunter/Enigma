...
function isAnswerCorrect(current, mode, answer) {
  if (!current) return false;
  const normalizedInput = normalizeInput(answer);
  const normalizedCorrect = normalizeInput(current[mode.answer]);
...
export default function PeriodicTableQuiz() {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      const mod = await import('@datasets/periodic_table.json');
      if (!mounted) return;
      setData(mod.default);
    })().catch((e) => {
  ...
  const checkAnswer = useCallback((answer) => {
    ...
    const isCorrect = isAnswerCorrect(current, mode, answer);
