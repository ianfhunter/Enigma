...
function findNonOverlappingSolution(wordPaths) {
  ...
  for (const [word, paths] of wordPaths) {
    for (const path of paths) {
      allPaths.push({ word, path, cells: new Set(path) });
    }
  }

  // Sort by path length descending (prefer longer words)
...
function generateSolvablePuzzle(baseSeed) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (solvable) {
      // Convert wordPaths map to sorted array of words
      const validWords = Array.from(wordPaths.keys())
  ...
  // Fallback: return last generated puzzle anyway (shouldn't happen often)
  ...
  return { letters, validWords, wordPaths, solution: [] };
}
