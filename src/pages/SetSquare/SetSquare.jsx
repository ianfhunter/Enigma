...
import { buildDilatedRows, buildColumnFooter, generateSetSquarePuzzle, isSolved } from './SetSquare.utils';
    ...
    <div className={styles.container}>
      <GameHeader
        ...
        instructions={t('games.setSquare.instructions', {
          defaultValue: 'Each row is shown as NUM OP NUM OP NUM = result. Fill the number cells with digits 1–9 once each. Column totals and their operators are shown in the footer.',
        })}
      />
      ...
      <div className={styles.boardWrap}>
        <div className={styles.expressionBoard}>
          {displayRows.map((row, rowIndex) => (
            <div className={styles.expressionRow} key={`row-${rowIndex}`}>
          ...
          <div className={styles.footerRow}>
            {footerCells.map((cell, idx) => {
              if (cell.kind === 'target') return <div key={`f-${idx}`} className={styles.columnTarget}>= {cell.value}</div>;
              return <div key={`f-${idx}`} className={styles.footerOperator}>{cell.value}</div>;
            })}
            <div className={styles.footerHint}>{t('games.setSquare.columnHint', { defaultValue: 'Column math (top→bottom, left-to-right):' })}</div>
          </div>
        </div>
      </div>
