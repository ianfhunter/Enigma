...
import { buildDilatedMatrix, generateSetSquarePuzzle, isSolved } from './SetSquare.utils';
...
export default function SetSquare() {
  ...
  const displayMatrix = buildDilatedMatrix(grid, puzzle.rowEquations, puzzle.colEquations);
    ...
    <div className={styles.container}>
      <GameHeader
        ...
        instructions={t('games.setSquare.instructions', {
          defaultValue: 'Fill the 3×3 number cells with digits 1–9 once each. The 5×5 board shows operators between cells; evaluate each row and column left-to-right to match the targets.',
        })}
      />
      ...
      <div className={styles.boardWrap}>
        <div className={styles.dilatedBoard}>
          {displayMatrix.map((row, rowIndex) => row.map((cell, colIndex) => {
            if (cell.kind === 'number') {
            ...
            const classByType = cell.kind === 'rowOp' ? styles.rowOperator : cell.kind === 'colOp' ? styles.colOperator : styles.junction;
            return <div key={`${rowIndex}-${colIndex}`} className={`${styles.operatorCell} ${classByType}`}>{cell.value}</div>;
          }))}

          {puzzle.rowEquations.map((equation, rowIndex) => (
            <div key={`row-target-${rowIndex}`} className={styles.rowTarget} style={{ gridRow: rowIndex * 2 + 1, gridColumn: 6 }}>
              = {equation.target}
            </div>
          ))}

          {puzzle.colEquations.map((equation, colIndex) => (
            <div key={`col-target-${colIndex}`} className={styles.colTarget} style={{ gridRow: 6, gridColumn: colIndex * 2 + 1 }}>
              = {equation.target}
            </div>
          ))}
        </div>
