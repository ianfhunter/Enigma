...
describe('Set Square helpers', () => {
  ...
  it('builds a 5x5 dilated matrix with operators between number cells', () => {
    const grid = [
      [1, 2, 3],
      [4, 5, 6],
      [7, 8, 9],
    ...
    const matrix = buildDilatedMatrix(grid, rowEquations, colEquations);
    expect(matrix).toHaveLength(5);
    expect(matrix[0]).toHaveLength(5);
    expect(matrix[0][0]).toMatchObject({ kind: 'number', value: 1 });
    expect(matrix[0][1]).toMatchObject({ kind: 'rowOp', value: '+' });
    expect(matrix[1][0]).toMatchObject({ kind: 'colOp', value: '+' });
    expect(matrix[1][1]).toMatchObject({ kind: 'junction' });
    expect(matrix[4][4]).toMatchObject({ kind: 'number', value: 9 });
  });
