...
describe('Set Square helpers', () => {
  ...
  it('builds dilated rows and column footer display models', () => {
    const grid = [
      [1, 2, 3],
      [4, 5, 6],
      [7, 8, 9],
    ...
    const rows = buildDilatedRows(grid, rowEquations);
    expect(rows).toHaveLength(3);
    expect(rows[0].cells.map(cell => cell.value)).toEqual([1, '+', 2, '-', 3]);
    expect(rows[0].target).toBe(2);

    const columnOpRows = buildColumnOperatorRows(colEquations);
    expect(columnOpRows).toEqual([
      [
        { kind: 'colOp', col: 0, value: '+' },
        { kind: 'spacer', value: '' },
        { kind: 'colOp', col: 1, value: '*' },
        { kind: 'spacer', value: '' },
        { kind: 'colOp', col: 2, value: '-' },
      ],
      [
        { kind: 'colOp', col: 0, value: '+' },
        { kind: 'spacer', value: '' },
    ...
    const footer = buildColumnFooter(colEquations);
    expect(footer).toEqual([
      { kind: 'target', value: 12 },
      { kind: 'target', value: 2 },
      { kind: 'target', value: 6 },
    ]);
  });
