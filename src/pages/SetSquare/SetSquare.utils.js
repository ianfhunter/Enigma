...
function countSolutions({ puzzleGrid, rowEquations, colEquations, maxSolutions = 2 }) {
  ...
  function backtrack(index = 0) {
    ...
    if (index === 9) {
      if (areAllEquationsSatisfied(grid, rowEquations, colEquations)) {
        solutions++;
      }
      return;
    }
    ...
    for (const digit of DIGITS) {
      ...
      if (isPlacementLocallyValid(grid, rowEquations, colEquations, row, col)) {
        backtrack(index + 1);
      }
  ...
  if (grid.flat().every(value => value !== 0) && !areAllEquationsSatisfied(grid, rowEquations, colEquations)) {
    return 0;
  }

  for (let row = 0; row < 3; row++) {
    for (let col = 0; col < 3; col++) {
      if (grid[row][col] !== 0 && !isPlacementLocallyValid(grid, rowEquations, colEquations, row, col)) {
        return 0;
      }
    }
  }
...
function buildDilatedMatrix(grid, rowEquations, colEquations) {
  return Array.from({ length: 5 }, (_, displayRow) => (
    Array.from({ length: 5 }, (_, displayCol) => {
      const row = Math.floor(displayRow / 2);
      const col = Math.floor(displayCol / 2);

      if (displayRow % 2 === 0 && displayCol % 2 === 0) {
        return { kind: 'number', row, col, value: grid[row][col] };
      }

      if (displayRow % 2 === 0 && displayCol % 2 === 1) {
        return { kind: 'rowOp', value: rowEquations[row].operations[Math.floor(displayCol / 2)] };
      }

      if (displayRow % 2 === 1 && displayCol % 2 === 0) {
        return { kind: 'colOp', value: colEquations[col].operations[Math.floor(displayRow / 2)] };
      }

      return { kind: 'junction', value: '•' };
    })
  ));
}

function formatEquation(equation, axis, index) {
  const label = `${axis}${index + 1}`;
  return `${label}: □ ${equation.operations[0]} □ ${equation.operations[1]} □ = ${equation.target}`;
}
