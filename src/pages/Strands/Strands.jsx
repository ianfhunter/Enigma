...
function generatePuzzleGrid(puzzle, random, maxAttempts = 100) {
  ...
  if (totalNeeded !== TOTAL_CELLS) {
    console.warn(`Strands: Word letters (${totalNeeded}) don't match grid cells (${TOTAL_CELLS}), will have gaps`);
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (emptyCellCount === 0) {
      console.log(`Strands: Successfully generated grid on attempt ${attempt + 1}`);
      return { grid, wordPositions };
    }
  }

  // Fallback: If we couldn't fill the grid perfectly, return best effort with random fill
  console.warn(`Strands: Could not generate perfect puzzle after ${maxAttempts} attempts, using fallback`);
...
function generatePuzzleFromCategories(seed) {
  ...
  for (const categoryKey of shuffledKeys) {
    ...
    if (totalLetters === TOTAL_CELLS) {
      console.log(`Strands: Selected ${themeWords.length} words for category "${category.name}": ${themeWords.join(', ')} (${totalLetters} letters)`);
      return {
        theme: category.name,
      ...
      console.warn(`Strands: Category "${category.name}" - letters don't match: ${totalLetters} vs ${TOTAL_CELLS}`);
    }
  }

  // Fallback: try harder with combined categories
  console.warn('Strands: Primary selection failed, trying combined word pool');

  // Combine words from multiple categories
  const firstCategory = wordCategories[shuffledKeys[0]];
  const spangram = CATEGORY_SPANGRAMS[shuffledKeys[0]];
  ...
  console.log(`Strands fallback: ${themeWords.length} words, ${totalLetters} letters (need ${TOTAL_CELLS})`);
        ...
        <div className={styles.wordList}>
          <div className={styles.wordListHeader}>
            <span className={`${styles.spangramLabel} ${gaveUp ? styles.revealed : ''}`}>
              Spangram: {(foundSpangram || gaveUp) ? puzzle.spangram : '_ '.repeat(puzzle.spangram.length)}
            </span>
          </div>
