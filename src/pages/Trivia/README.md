# 🧠 Trivia

General knowledge quiz across many topics!

## Category

Trivia & Knowledge

## How to Play

1. Questions are presented from various categories
2. Select or type the correct answer
3. Categories include history, science, sports, and more
4. Score points for correct answers
5. Challenge yourself across all topics

## Puzzle Source

**📊 Dataset**

Puzzles are loaded from a curated dataset.

**Source:** `datasets/trivia_datasets/` - categorized trivia questions

## Tips & Strategy

- Read questions carefully before answering
- Eliminate obviously wrong options first
- General knowledge builds over time

## Issues & Bugs

🔍 [Search existing issues for "Trivia"](https://github.com/ianfhunter/Enigma/issues?q=is%3Aissue+Trivia)

📝 [Report a new bug](https://github.com/ianfhunter/Enigma/issues/new?labels=bug&title=%5BTrivia%5D+)

---

*Part of the [Enigma](https://github.com/ianfhunter/Enigma) puzzle collection*
