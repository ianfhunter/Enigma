...
function chordCross(a, b, c, d) {
  ...
  // Shared endpoints never count as crossings here
  if (a === c || a === d || b === c || b === d) return false;
  // Treat chords as crossing when their numeric ranges overlap (excluding shared endpoints).
  return a < d && c < b;
}
...
function makeOuterPlanarGraph(n, extraEdges = Math.floor(n / 2)) {
  ...
  const add = (u, v) => {
    ...
    // Always allow perimeter edges (adjacent in the cycle) without crossing checks
    if (diff === 1 || diff === n - 1) {
      edges.add(key);
      return true;
    }
