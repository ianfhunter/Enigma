...
import { createSeededRandom, getTodayDateString, stringToSeed, getWordsByLength } from '../../data/wordUtils';
...
function generateWaffle(seed, size) {
  ...
  const words = getWordsByLength(size).slice(0, 2000);
  ...
  if (size === 5) {
    return {
      ...
      words: { h1: 'START', h2: 'ARENA', h3: 'STARS', v1: 'SEALS', v2: 'AREAS', v3: 'TESTS' },
      size: 5,
    };
  } else {
    return {
      ...
      words: { h1: 'ABRIDGE', h2: 'COASTAL', h3: 'READING', h4: 'SELECTS', v1: 'ACCORDS', v2: 'READABL', v3: 'DITAINC', v4: 'EXLINGS' },
      size: 7,
    };
  }
}
...
export default function Waffle() {
  ...
  const initGame = useCallback((gameSize) => {
    ...
    const seed = stringToSeed(`waffle${gameSize}-${today}`);
    const newPuzzle = generateWaffle(seed, gameSize);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <h1 className={styles.title}>Waffle</h1>
