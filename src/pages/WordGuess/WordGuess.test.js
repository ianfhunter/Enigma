...
// ===========================================
// validateStrictWordGuess Tests
// ===========================================
describe('validateStrictWordGuess', () => {
  it('should return null when there are no previous guesses', () => {
    expect(validateStrictWordGuess('CRANE', [], 5)).toBeNull();
  });

  it('should require keeping green letters in the same position', () => {
    const guesses = [
      { word: 'CRANE', feedback: ['correct', 'absent', 'absent', 'absent', 'absent'] },
    ];
    const violation = validateStrictWordGuess('TRAIL', guesses, 5);
    expect(violation).toEqual({ type: 'position', letter: 'C', index: 0 });
  });

  it('should require reusing discovered yellow letters', () => {
    const guesses = [
      { word: 'SLATE', feedback: ['absent', 'present', 'absent', 'absent', 'absent'] },
    ];
    const violation = validateStrictWordGuess('CRANE', guesses, 5);
    expect(violation).toEqual({ type: 'letters', letters: ['L'] });
  });

  it('should respect duplicate yellow/green letters', () => {
    const guesses = [
      { word: 'EERIE', feedback: ['present', 'present', 'absent', 'absent', 'present'] },
    ];
    const violation = validateStrictWordGuess('ELIDE', guesses, 5);
    expect(violation).toEqual({ type: 'letters', letters: ['E'] });

    const satisfied = validateStrictWordGuess('EERED', guesses, 5);
    expect(satisfied).toBeNull();
  });

  it('should pass when all strict requirements are met', () => {
    const guesses = [
      { word: 'CRANE', feedback: ['absent', 'present', 'present', 'absent', 'absent'] },
      { word: 'BLOOM', feedback: ['absent', 'absent', 'absent', 'absent', 'absent'] },
    ];
    expect(validateStrictWordGuess('FRAPS', guesses, 5)).toBeNull();
  });
});
