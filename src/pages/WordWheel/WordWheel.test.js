...
import { MODES } from './WordWheel.jsx';
import { generatePuzzle, findAllWords } from '../../data/wordUtils';

describe('WordWheel - helpers', () => {
  it('exposes modes', () => {
    expect(MODES.CLASSIC.letters).toBe(9);
  });

  it('wordUtils generatePuzzle produces letters and center', () => {
    const puzzle = generatePuzzle(5, 50, MODES.CLASSIC.letters);
    expect(puzzle.letters.length).toBe(MODES.CLASSIC.letters);
    expect(puzzle.center).toBeDefined();
    const words = findAllWords(puzzle.letters, puzzle.center);
